...
# Logic & Conditionals
3 > 4 # Logical statement. Asking whether 3 is greater than 4. It's FALSE
4 == 4 # TRUE
'three' == 'three' # works for strings too
5 != 4 # TRUE
'three' == 3 #F ALSE. strings have no numerical properties
...
x[1] # Navigates to first element of the vector
x[3] # Navigates to the third element
y[1] # First element
y[3] # Third element

# Use square brackets [] to navigate two-dimensional data as well
# [row, column]
mtx <- matrix(x, nrow = 3, ncol = 2) # a matrix can only be numeric
...
diff(c(mean(z), sd(z))) #returns the difference of the mean and standard deviation
...
your_data <- read.csv('file_name.txt') # Only works if file is in directory above

# Exporting Data
a <- seq(1,71,3) # goes from 1 to 71 in increments of 3. So 1,4,7,10... to 71
write(a, file = 'our_sequence.txt') #Exports 'a' as a file to our working directory



#-------------------------------------#
##WORKSHEET 5: For Loops
# For loops are used to repeat a function, or a set of functions multiple times

#Basic for loop
# The print() function below runs 3 times. Once when i = 1, again when i = 2,
# and again when i = 3. Each time it runs, it will run the same function the same
# way. The function we want repeated goes between the curly braces.
for(i in 1:3){
    print('hello world')
}
...
# Let's add a new column to the 'our_group' dataframe that has the state for each
# zip code.
our_group$state <- 0
...
for(i in 1:length(our_group)){
    if(our_group$zip_codes[i] == 94933){
        ...
        our_group$state[i] <- 'California'
        # hint: I made all the code below by copy and pasting the two lines
        # above and just changing the zip code & state
...
# There are too many customization option for ggplot for me to try explaining
...
# We still need to tell ggplot which type of plot we would like
...
# Save this cheat sheet an use it any time you're stuck on coding problems in
# the future :)
